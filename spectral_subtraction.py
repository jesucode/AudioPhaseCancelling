...
def spectral_subtraction(noise_profile_n, input_signal_y, dimension):
	...
	noise_mean = mN.astype("float64") #np.mean(mN, axis=1, dtype="float64") #find noise mean # # 
	#noise_mean = noise_mean[:, np.newaxis] # perform subtraction
	output_X = mY - noise_mean[: , :2577]
	#factor  = mY / noise_mean
	#factor  = 	np.average(mN) #find noise mean
	#print (factor)
	#output_X = mY - (noise_mean * factor)
